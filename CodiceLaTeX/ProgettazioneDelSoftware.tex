...
\begin{document}
    ...
    \subsection{Decomposizione in moduli}
        \subsubsection{Descrizione delle classi individuate}
            ...
            \begin{itemize}[label=$\bullet$]
                ...
                \item \textbf{Prestiti}: classe che modella un insieme di prestiti. Possiede l'attributo \texttt{prestiti} (una collezione di oggetti di tipo Prestito). Implementa l'interfaccia \hyperlink{Archiviabile}{Archiviabile}, ereditando i metodi fondamentali per la manipolazione della collezione. Fornisce un metodo per la visualizzazione dei prestiti (getter) e un metodo di filtraggio per visualizzare solo i prestiti che rispettano il criterio selezionato; 
                \item \textbf{Utenti}: classe che modella un insieme di utenti. Possiede gli attributi \texttt{chiaviMatricole} e \texttt{utenti} (due collezioni di oggetti di tipo Utente). Implementa le interfacce \hyperlink{Archiviabile}{Archiviabile} e \hyperlink{Mappabile}{Mappabile}, ereditando i metodi fondamentali per la manipolazione della collezione. Fornisce un metodo per la visualizzazione degli utenti (getter) e un metodo per effettuare una ricerca all’interno della collezione;
                \item \textbf{Libri}: classe che modella un insieme di libri. Possiede gli attributi \texttt{chiaviISBN} e \texttt{libri} (due collezioni di oggetti di tipo Libro). Implementa le interfacce \hyperlink{Archiviabile}{Archiviabile} e \hyperlink{Mappabile}{Mappabile}, ereditando i metodi fondamentali per la manipolazione della collezione. Fornisce un metodo per la visualizzazione dei libri (getter) e un metodo per effettuare una ricerca all’interno della collezione;
                \hypertarget{Archiviabile}{\item} \textbf{Archiviabile}: interfaccia parametrizzata implementata dalle classi Prestiti, Utenti e Libri. Definisce i tre metodi per le operazioni fondamentali di aggiunta, modifica e rimozione dell'elemento passato come parametro; 
                \hypertarget{Mappabile}{\item} \textbf{Mappabile}: interfaccia parametrizzata implementata dalle classi Utenti e Libri. Definisce i metodi per l'accesso e la ricerca di un valore attraverso una chiave;
                \item \textbf{Filtro}: classe enumerativa che definisce tre criteri di filtraggio: \texttt{TUTTI}, \texttt{ATTIVI} e \texttt{CONCLUSI}. Questa classe è utilizzata per l'applicazione di criteri di visualizzazione selettiva sull'insieme dei prestiti; 
            ...
            \end{itemize} 
            Per la gestione e segnalazione dei possibili errori che si possono verificare durante l'esecuzione delle operazioni fondamentali del software, sono state realizzate anche le classi \textbf{FormatoCampiErratoException} e \textbf{OggettoGiaPresenteException} (che si specializza in \textbf{UtenteGiaPresenteException} e \textbf{LibroGiaPresenteException}).\vspace{.2cm}
            \\All'interno del diagramma viene omessa l’implementazione dell’interfaccia Serializable al fine di aumentare la propria leggibilità. Suddetta interfaccia dovrà essere implementata dalle classi:\vspace{-0.2cm}
            \begin{itemize}[label=$\bullet$]
                ...
                \item ISBN.
            \end{itemize}
            Per lo stesso motivo, all'interno del diagramma viene omessa anche l'implementazione dell'interfaccia Comparable, che sarà implementata dalle classi:\vspace{-0.2cm}
            \begin{itemize}[label=$\bullet$]
                \setlength\itemsep{-0.3em}
                \item Prestito;
                \item Libro;
                \item Matricola;
                \item ISBN;
                \item Persona (con le proprie specializzazioni).
            \end{itemize}
        \newpage

        \subsubsection{Diagramma delle classi}
        \begin{figure}[h!]
            \begin{adjustbox}{width=.95\paperwidth, height=.7\paperheight, center, margin= 0cm .6cm 0cm .1cm}  
        ...
        \begin{figure}[h!]
            \begin{adjustbox}{width=.43\paperwidth, height=.4\paperheight, center, margin= 0cm .6cm 0cm .1cm}
        ...
        \begin{figure}[h!]
            \begin{adjustbox}{width=.57\paperwidth, height=.75\paperheight, center, margin= 0cm .3cm 0cm .1cm}
    ...
    \subsection{Diagrammi di sequenza}
        ...
        \subsubsection{C4: Inserimento di un libro}
        ...
        \subsubsection{C5: Modifica di un libro}
        ...
        \subsubsection{C6: Rimozione di un libro}
        ...
        \subsubsection{C7: Inserimento di un utente}
        ...
        \subsubsection{C8: Modifica di un utente}
        ...
        \subsubsection{C9: Rimozione di un utente}
        ...
        \subsubsection{C10: Ricerca di un libro}
        ...
        \subsubsection{C12: Ricerca di un utente}
        ...
        \subsubsection{C13: Registrazione di un prestito}
        ...
        \subsubsection{C14: Registrazione di una restituzione}
        ...
        \subsubsection{C15: Estensione di un prestito}
    ...
    \subsection{Principi di buona progettazione}
        ...
        \subsubsection{Strategie per la riduzione dell'Accoppiamento}
            ...
            \\Una possibile soluzione potrebbe essere quella di predilire l'\emph{associazione} all'\emph{ereditarietà}: anziché far dipendere l'implementazione di \emph{Utente} interamente da quella di \emph{Persona}, si potrebbe pensare di far mantenere a \emph{Utente} un riferimento ad una classe avente come attributi \emph{nome} e \emph{cognome} (e che potrebbe prendere il nome di "Anagrafica"), riducendo di molto l'accoppiamento.
